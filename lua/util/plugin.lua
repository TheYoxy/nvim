...
M.deprecated_extras = {
  ["my-lazy.plugins.extras.formatting.conform"] = "`conform.nvim` is now the default **LazyVim** formatter.",
  ["my-lazy.plugins.extras.linting.nvim-lint"] = "`nvim-lint` is now the default **LazyVim** linter.",
  ["my-lazy.plugins.extras.ui.dashboard"] = "`dashboard.nvim` is now the default **LazyVim** starter.",
  ["my-lazy.plugins.extras.coding.native_snippets"] = "Native snippets are now the default for **Neovim >= 0.10**",
  ["my-lazy.plugins.extras.ui.treesitter-rewrite"] = "Disabled `treesitter-rewrite` extra for now. Not ready yet.",
  ["my-lazy.plugins.extras.ui.treesitter-main"] = "the `nvim-treesitter` main branch is now used by default",
  ["my-lazy.plugins.extras.coding.mini-ai"] = "`mini.ai` is now a core LazyVim plugin (again)",
  ["my-lazy.plugins.extras.lazyrc"] = "local spec files are now a lazy.nvim feature",
  ["my-lazy.plugins.extras.editor.trouble-v3"] = "Trouble v3 has been merged in main",
  ["my-lazy.plugins.extras.lang.python-semshi"] = [[The python-semshi extra has been removed,
  because it's causing too many issues.
  Either use `basedpyright`, or copy the [old extra](https://github.com/LazyVim/LazyVim/blob/c1f5fcf9c7ed2659c9d5ac41b3bb8a93e0a3c6a0/lua/my-lazy/plugins/extras/lang/python-semshi.lua#L1) to your own config.
  ]],
}
...
function M.extra_idx(name)
  ...
  for i, extra in ipairs(Config.spec.modules) do
    if extra == "my-lazy.plugins.extras." .. name then
      return i
    end
  end
end
